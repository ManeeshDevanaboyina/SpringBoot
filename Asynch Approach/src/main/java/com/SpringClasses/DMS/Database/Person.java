...
@NamedQueries(value= {
		...
		//Named Query1
		@NamedQuery(name="Person.giveDataByName",
				query="select p from Person p where p.firstName=?1"),
		
		@NamedQuery(name="Person.giveFewColumns",
		query="select new com.SpringClasses.DMS.Database.customType(p.firstName,p.lastName)" + "from Person p where p.firstName=?1"),
})
public class Person {
